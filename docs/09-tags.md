...
`/tags` に、使っているタグを Item 件数つきで並べる。袖（[03-functional-spec.md](03-functional-spec.md) 3.9）の「タグ」から入れる。
`g` `s`（[08-todo-management.md](08-todo-management.md) 8.4）は、この一覧を経由せず
タグを選ぶダイアログから直接タスク一覧へ移る別の入口。

- タグを押すと、そのタグで絞り込んだタスク一覧（`/?tag=`）へ移る。
  行き先は未完了側（8.4「完了の表示」）なので、完了済みは並ばない
...
- タグとして拾う `#` は、**行頭か空白の直後にあるものだけ**（`isTagStart`）。
  語の途中の `#` は書いた文字のまま残す。どこにあっても拾うと、URL の中の
  `#`（`https://www.chatwork.com/#!rid1383…`）まで拾って、書いた覚えのない
  タグができる。本文の記法（`#タグ`。
  [11-scrapbox-notation.md](11-scrapbox-notation.md)）と同じ切り方でもある
- `#` の直後から、空白までをタグ名として扱う
- 複数指定できる（`#買い物 #急ぎ`）
- 存在しないタグ名なら、その場で作成する
- タグ名に使えない文字（空白・カンマ）が来たらそこで区切る
- **同じ入力にある URL が指すものの ID はタグにしない**（`collectUrlIds`）。
  `#xxx` の `xxx` が、入力（2行目以降も含む）の URL のパスの区切り1つ、
  またはフラグメントと**完全に一致**すれば、書いた文字のままタイトルに残す
...
ClickUp などのタスクを「タイトル | #ID」と URL で貼ると、ID がタグになって
しまうため。ID はチケットの番号であって分類ではない。部分一致は見ない。
`#web` と `https://example.com/web-app` のような偶然の重なりでタグが消えないように。
...
| 操作 | 動作 |
|---|---|
| `#` | すべてのタグを出す（多いときは先頭から8件）。タグにならない `#`（語の途中）では出さない |
| 続けて入力 | 名前に含むものへ絞り込む |
| `↑` `↓` | 選ぶ |
