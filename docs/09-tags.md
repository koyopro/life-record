...
- タグを押すと、そのタグで絞り込んだタスク一覧
  （`/items?status=all&open=true&tag=`）へ移る。`open=true` で完了済みを除く
  （件数は status を問わず数えているため、混じっていると一覧の表示件数とは
  食い違って見えることがある）
...
タグの絞り込みは**すべての一覧で使えるようにする**（今日 / Inbox / タスク）。
...
|---|---|
| `s` | 選択中のタスクのタグを変更 |
| `g` `s` | タグを選ぶダイアログを開く。タグ名はインクリメンタルに絞り込まれ、選ぶとそのタグのタスク一覧（`/items?status=all&tag=`）へ移る |
