...
- [x] タスクのメモ（`m`。日付を持たない覚え書き。繰り返しの次回へ引き継ぐ）
...
- [x] 当日の Section をすばやく追加する動線（`y` で当日の枠へ移る）
...
## Milestone 13: ホーム画面アイコンからの導線（完了）

ホーム画面のアイコンを長押しして、そのまま書き始められるようにする。
詳細は [14-app-shortcuts.md](14-app-shortcuts.md)。

- [x] manifest の `shortcuts`（「タスクを追加」→ `/add`、
      「今日の日記を開く」→ `/diary/today`）
- [x] 追加だけをする画面（`app/pages/add.vue`。一覧と同じ入力欄を使う）
- [x] 開いた時点で入力欄へフォーカスする（`ItemComposer` の `autofocus`）
- [x] 追加は一覧・共有と同じ経路（オフラインでも追加でき、後から送られる）
- [x] その日の日記へ振り分ける経路（`app/pages/diary/today.vue`。
      manifest には日付を書けないため）
- [ ] Android 実機での確認（[14-app-shortcuts.md](14-app-shortcuts.md) 14.5）

---

## Milestone 14: 自分で登録するアイコン（完了）

絵文字だけでは足りない印を、自分で登録して本文に置けるようにする
（Notion のカスタム絵文字にあたる）。詳細は
[11-scrapbox-notation.md](11-scrapbox-notation.md) 11.8。

- [x] `icons` テーブル（名前と画像の場所だけを持つ）
- [x] 登録・削除の API（画像は本文中の画像と同じ経路で S3 へ）
- [x] 本文の `:name:` を画像として描く（登録が無ければ文字のまま）
- [x] `:` の候補に混ぜる（登録したものを先に、絵文字を後に）
- [x] 管理画面（`/icons`）

---

## Milestone 15: Section・日記のオフライン対応（完了）

作業記録（Section）と日記も IndexedDB へ置き、オフラインで読み書きできる
ようにする（[12-offline.md](12-offline.md)）。

- [x] `sections` / `diaries` ストア（Item ごと・日付ごとに引ける）
- [x] `PUT /api/sections/:id`（id はクライアントが決める upsert）
- [x] 本文の未送信の操作を列へ積む（対象ごとに1つ・送信時に手元から取り直す）
- [x] サーバーから取り直した内容の重ね方（未送信を上書きしない・古い応答で戻さない）
- [x] 日記の「この日にやったこと」を手元の作業記録から作る
- [x] 起動時に、送れていない本文を積み直す

---

## Milestone 16: 別の日の作業記録（完了）

作業した日を後から書けるようにする（[03-functional-spec.md](03-functional-spec.md) 3.2）。

- [x] 「作業記録を追加」で日付を選んで作れる（当日固定の追加ボタンは廃止）
- [x] 作った記録をその場で開いてフォーカスする
- [x] 当日の枠の日付も、日付を押せば直せる
- [x] 1つのタスクの記録は同じ日に1件だけにする（すでにある日は作らずそこへ移り、
      日付を移した先に記録があれば1つにまとめる）

---

## Milestone 17: 日記の振り返りやすさ（完了）

- [x] 「この日にやったこと」に、その日の作業記録を冒頭5行まで添える
- [x] 状態ごとではなく、タグごとにまとめて出す（タグなしが先頭）
- [x] 作業メモの無い Item は出さない

---

## Milestone 18: 書き出しと取り込み（完了）

機能を足した分（タグの色・アイコン）が書き出しから抜けていたのを直し、
書き出した JSON を取り込み直せるようにする（[05-operations.md](05-operations.md) 5.3 / 5.4）。

- [x] JSON にタグ（色・未使用のタグ）とアイコンを含める
- [x] テキストにアイコンの対応表を添える（本文の `:name:` を読めるように）
- [x] 日記を日時ごと書き出す
- [x] `npm run import:backup` で書き出した JSON から戻せる（何度流しても同じ結果）

---

## Milestone 19 以降（将来検討）
