...
曜日で決まるもの（`月曜日以降`。10.7）は、完了日から見て次にその曜日が来る日。

```text
「月曜日以降」を 9/2（水）に完了 → 次回期限は 9/7（次の月曜）
```
...
| `*after monday` / `*月曜日以降` / `*完了後の次の月曜` | `FREQ=WEEKLY;BYDAY=MO` | completion |
| `*after weekday` / `*平日以降` / `*完了後の次の平日` | `FREQ=WEEKLY;BYDAY=MO,TU,WE,TH,FR` | completion |

- `完了の` は省略できる（`3日後` だけで `完了の3日後` と同じ）。「後」は
  due（毎〜）側では使わない語なので、省略しても曖昧にならない
- 「ヶ月」「か月」とも書ける（`完了の1ヶ月後` `2ヶ月ごと`）。表示でこの言い方を
  するので、書き戻した文をそのまま読み直せる必要がある

#### 完了日起点で、曜日で決まるもの

`月曜日以降`（RTM の after monday）は、**完了した日から見て次に来るその曜日**が
次回の期限になる。間隔（`完了の1週間後`）では表せないので、別の言い方として
受け取る。

```text
「月曜日以降」を水曜（9/2）に完了 → 次回期限は 9/7（次の月曜）
```

表示は `完了後の次の月曜`。**曜日を落として「完了の1週間後」と出さない**
（完了した曜日の1週間後、という別の意味になってしまう）。`平日` `週末` の
組（10.7「月の中の並びで決まる指定」と同じ言い方）も受け取る。
