...
WebView のスクロールバーは、iOS のように**重ねて出るとは限らない**。
macOS は「スクロールバーを常に表示」やマウスをつないだときに、**幅を
その分だけ削るバー**を出す。本文は中央に寄せてある（`app.vue` の `.shell`）
ので、バーが出たり消えたりするたびに画面全体の横位置がその半分だけ動く。
一覧を `j` / `k` で辿ると、作業記録が長く縦にはみ出すタスクに当たったときだけ
各要素が横へずれることになる。

そこで、**スクロールする場所は、バーが出ていないあいだもそのぶんを空けておく**
（`scrollbar-gutter: stable`）。

- 画面そのもの（`app/assets/css/main.css` の `html`）
- 分割表示の右ペイン（`ListDetailSplit`）。一覧とは別にスクロールするので、
  タスクを送るたびにバーが出入りする
  （[03-functional-spec.md](03-functional-spec.md) 3.1「分割表示（PC）」）

重ねて出るバー（iOS や macOS の既定）は幅を取らないので、そちらではこの指定は
何もしない（余白も空かない）。`scrollbar-gutter` を知らない古い WebView
（Safari 18.1 以前）では、代わりに常に出しておく（`@supports not` で
`overflow-y: scroll`）。どちらでも幅は動かない。
