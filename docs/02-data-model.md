...
本文から他のページを指すリンク（`[/items/<id>]`、`[/diary/YYYY-MM-DD]`、
`[/diary/month/YYYY-MM]`）についても、**関連を保存するテーブルは持たない**。
保存されるのは本文に書かれた1方向のリンクだけで、逆向き（そのページを
指しているものの一覧＝バックリンク）は、その文字列を含む本文を探して
導き出す（[11-scrapbox-notation.md](11-scrapbox-notation.md) 11.11）。

日付で結ぶ 5 と同じ考え方で、**片側を消したときに反対側が残る、という
食い違いが起こらない**。月の振り返りのように「カレンダーの月と、それに
ついてのTODO」を相互に行き来したい場合も、この仕組みだけで足りる。
専用のカラム・テーブル・タグは作らない。
...
- Item 上で、そのItemに関連する Diary へのリンクを日付ごとに表示する
  （本文からの相互リンクは 2.10 の 6 で入れた。ここは日付からの自動導出の話）
- タグの階層化・タググループ
