...
useShortcuts(
  computed<Shortcut[]>(() => [
    ...
    {
      /*
       * 検索へ移る（一覧と同じ `/`）。書きながら思い出したことを、
       * いったんタスク一覧へ出てから探し直さずに済むようにする。
       *
       * ここでも app.vue には置かない。ページは非同期に読み込まれるので
       * app.vue の登録のほうが先になり、検索画面自身の `/`（検索語を
       * 打ち直す）を追い越してしまう（ItemListView と同じ理由。
       * docs/08-todo-management.md 8.4）。
       *
       * 移動は leaveTo で行う。書きかけを送る合図を出してから離れる
       * （日付の移動・カレンダーと同じ）。
       */
      keys: ['/'],
      label: '検索',
