...
/**
 * `[URL]` で書く埋め込み（docs/11-scrapbox-notation.md 11.12）。
 *
 * 外部の Web ページを、その場に iframe として出す。**埋め込みにする URL は
 * ホスト名で決める**（`IFRAME_HOSTS`）。`[URL]` は外部リンクの記法でもあり、
 * どの URL も埋め込みにすると普通のリンクが書けなくなるため。
 *
 * 高さは記法からは決めていない（いまは既定値だけ）。あとから指定できるよう、
 * 節点には持たせておく。
 */
