...
/**
 * 埋め込み（`[URL]`）を出す（docs/11-scrapbox-notation.md 11.12）。
 *
 * URL は**属性としてだけ**渡す。ユーザーが書いた文字列なので、エスケープの
 * うえで `src` に入れ、HTML として解釈させる余地を作らない（11.10）。
 * パーサーは `http(s)` の、決めたホストのものしか埋め込みにしないが、
 * ここでも通す URL を確かめる（描画側だけを見て安全と分かるようにするため）。
 *
 * 横幅は本文の幅いっぱい（CSS）。高さは節点の値をそのまま px で入れる。
 * 数値としてしか作らないので属性に入れても安全。
 */
function renderIframe(node: IframeNode): string {
